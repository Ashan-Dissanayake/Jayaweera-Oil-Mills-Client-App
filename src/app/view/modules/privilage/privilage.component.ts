...
export class PrivilageComponent {
  ...
  generateAuthority() : void{
    const module = this.form.controls['module'].getRawValue().name;
    const operation = this.form.controls['operation'].getRawValue().name;
    ...
    if (errors != "") {
      ...
      if (updates != "") {
        ...
        const confirm = this.dg.open(ConfirmComponent, {
          width: '500px',
          data: {
            heading: "Confirmation - Privilage Update",
